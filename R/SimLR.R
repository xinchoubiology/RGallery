...
##' 
##' @title fast_LR_sim
##' @description A fast data simulation function implemented by Rcpp.
##'              And we will change the data set size for L2-regularized 
##' @param N number of observations
##' @param p number of predictors
##' @param rho correlation of X_j and X_{j'}. Default is 0.95
##' @param snr signal-noise ratio threahold. Default is 3.0
...
##' @return beta p real beta value
##' @author Eric xin Zhou \url{xxz220@@miami.edu}
fast_LR_sim <- function(N, p, rho = 0.95, snr = 3.0, nmu = 0.0, nsd = 1.0, control = TRUE){
  # multinormal matrix generation subject to cor(xi,xj) == 0.95
  ## sigma build
  sigma <- as.vector(diag(N))
  sigma[which(sigma == 0)] <- rho
  ## x N x p
  x <- t(cmvrnorm(n = p, mu = rep(0, N), Sigma = matrix(sigma, nrow = N)))
  ## noise generation
  z <- rnorm(N)
  ## beta generation
  beta <- genBeta(p)
